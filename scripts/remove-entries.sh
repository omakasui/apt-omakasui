...
# Usage: remove-entries.sh --package <name> --product <product> --suite <suite>

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/lib/targets.sh"

PACKAGE="" PRODUCT="" SUITE=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -n "$PACKAGE" ]] || { echo "ERROR: --package is required"; exit 1; }
target_require "$PRODUCT" "$SUITE"
[[ -f index/packages.tsv ]] || exit 0

before=$(wc -l < index/packages.tsv); tmp=$(mktemp)
awk -v product="$PRODUCT" -v suite="$SUITE" -v package="$PACKAGE" '
  {
    selected=($1==product && $2==suite)
    if (!(selected && $4==package)) print
  }
