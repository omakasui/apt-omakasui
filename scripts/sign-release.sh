...
# Usage: sign-release.sh [--targets "product/suite ..."] (--key-id <fingerprint> | --key-url <url>)
...
for target in $TARGET_FILTER; do
  ...
  for published_suite in "$suite" "${suite}-dev"; do
    dir="${product}/dists/${published_suite}"
    apt-ftparchive \
      ...
      -o APT::FTPArchive::Release::Description="${label} packages" \
